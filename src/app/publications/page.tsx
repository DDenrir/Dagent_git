...
export default function PublicationsPage() {
  const papers = [
    {
      id: 1,
      title:
        'Supply Chain Transparency and Risk Management in Community Enterprises: A Case Study of Phetchabun Province',
  ...
  const getBadgeClass = (type: string) => {
    switch (type) {
      case 'TCI 1':
        return styles.badgeTci1;
      case 'TCI 2':
        return styles.badgeTci2;
      case 'Scopus':
        return styles.badgeScopus;
      default:
        return '';
    }
  };
            ...
            {papers.map((paper) => (
              <div key={paper.id} className={`${styles.journalCard} ${getTypeClass(paper.type)}`}>
                ...
                <div className={styles.metaInfo}>
                  ...
                  <span>
                    Vol. {paper.volume}, No. {paper.year}
                  </span>
                  <span>pp. {paper.pages}</span>
                  <span>
                    <i className="fa fa-quote-right"></i> Cited by {paper.citation}
                  </span>
                </div>
